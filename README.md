...
```jsonc
{
  ...
  "default_agent": "sporeflow",
  "agent": {
    "sporeflow": {
      "temperature": 0.1,
      "prompt": "{file:./sporeflow.md}"
    }
  }
}
```

Similarly, **Claude Code** also has a custom agent configuration which can be used to automate the protocol injection. See the configuration guide [here](https://code.claude.com/docs/en/sub-agents).
...
### 3. Watching & Syncing
After adding `agent.md` charm to the `spores/`, you can tell the agent to "Keep watchers open." This instructs the agent to monitor changes in your Spore files and automatically update the code in `/root/` whenever you modify your intent.

### 4. The Self-Contained IDE
If you prefer a web-based interface for managing your spores without a local editor, after adding `ide.md` charm to the `spores/`, ask the agent to "Build the self-contained Golang IDE."
